...
string run_test(string filename, string settings, string outpfile,
  ...
  cout << "running ./wanda.exe " << settings << " " << filename
       << " ... ";
  cout.flush();
  execute("timeout " TIMEOUT " ./wanda.exe " + settings + " " +
...
void htmlify_input_file(int number) {
  ...
  execute("./converter.exe " + files[number] +
          " -f html > evaluation/details/" + filename + ".html");
}
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    cout << "First, a number of entries a : b, where a is the title "
         ...
         << "to WANDA.  Each of these entries should be on a "
    ...
    cout << "Then, a number of entries of the form title : query, "
         ...
         << "WANDA (something like \"local\" or \"leftlinear\".  "
    ...
    cout << "Invoke EVALUATOR with as arguments the files on which "
         << "you want to run WANDA (or a directory, in which you "
  ...
  system("cp support/afs.css evaluation/details");
